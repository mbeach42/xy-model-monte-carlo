...
@everywhere function energy_singleflip_XY(M::Array{Float64,2}, deltaTheta::Float64, i::Int64, j::Int64, L::Int64)::Float64 #energy of configuration
    E, Etrial = 0.0, 0.0
    Etrial = -cos(M[i,j]-M[i, periodic_index(j+1,L)])
            - cos(M[i,j]-M[i, periodic_index(j-1, L)])
            - cos(M[i,j]-M[periodic_index(i+1, L),j])
            - cos(M[i,j]-M[periodic_index(i-1, L),j])
    M[i,j] -= deltaTheta
    E = -cos(M[i,j]-M[i, periodic_index(j+1,L)])
            - cos(M[i,j]-M[i, periodic_index(j-1, L)])
            - cos(M[i,j]-M[periodic_index(i+1, L),j])
            - cos(M[i,j]-M[periodic_index(i-1, L),j])
    M[i,j] += deltaTheta
...
@everywhere function MC_step_XY!(config::Array{Float64,2}, T::Float64, L::Int64,
    ...
    if r > min(1.0, exp(-1.0/T*deltaE)) #if spin flip is favourable, accept new state
        config[x, y] -= deltaTheta #otherwise reverse the spin flip
    end
end

@everywhere function MC_sweep_XY!(config::Array{Float64,2}, T::Float64, L::Int64, x::Array{Int64, 1}, y::Array{Int64, 1}, rs::Array{Float64, 1}, deltaTheta::Array{Float64, 1}) #Sweep over L^2 MC steps
    for i = 1:2*L^2
