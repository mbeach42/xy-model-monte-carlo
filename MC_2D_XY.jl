...
function periodic_index(a::Int64, L::Int64)::Int64 #This impliments periodic boudnary conditions (PBC's)
 ...
 function random_config_XY(L::Int64)::Array{Float64,2} #generate a random (L,L) matrix of +/-1
    return 2.0*pi*rand(L,L)
end

 function energy_singleflip_XY(M::Array{Float64,2}, deltaTheta::Float64, i::Int64, j::Int64, L::Int64)::Float64 #energy of configuration
   E, Etrial = 0.0, 0.0
   Etrial = - cos(M[i,j]-M[i, periodic_index(j+1,L)]) - cos(M[i,j]-M[i, periodic_index(j-1, L)]) - cos(M[i,j]-M[periodic_index(i+1, L),j]) - cos(M[i,j]-M[periodic_index(i-1, L),j])
   M[i,j] -= deltaTheta
 ...
 function energy_XY(M::Array{Float64,2}, L::Int64)::Float64 #energy of configuration
   E = 0.0
 ...
 function stiffness_XY(M::Array{Float64,2}, T::Float64, L::Int64)::Float64 # in y-direction
   term1, term2 = 0.0, 0.0
    for j = 1:L, i = 1:L
 ...
 function MC_step_XY!(config::Array{Float64,2}, T::Float64, L::Int64,
 ...
 function MC_sweep_XY!(config::Array{Float64,2}, T::Float64, L::Int64, x::Array{Int64, 1}, y::Array{Int64, 1}, rs::Array{Float64, 1}, deltaTheta::Array{Float64, 1}) #Sweep over L^2 MC steps
   for i = 1:2*L^2
       MC_step_XY!(config, T, L, x[i], y[i], rs[i], deltaTheta[i])
   end
end

 function thermo_quantities_XY(T::Float64, L::Int64, N_eq::Int64, N_steps::Int64)#::Tuple{Float64,Float64,Float64}
 ...
 function iterate_over_temperatures_XY(Ts::Array{Float64, 1}, L::Int64,
